#include "scp_console_menu.h"

#include <limits>
#include <sstream>

scp::ConsoleMenu::ConsoleMenu(
...
void scp::ConsoleMenu::operator()() const {
  uint32_t userChoice;
  out_ << '\n';
...
void scp::ConsoleMenu::SetCurrentFile(const std::string& filename) {
...
void scp::ConsoleMenu::PrintVector(const std::vector<uint32_t>& v,
...
void scp::ConsoleMenu::PrintMatrix(
...
void scp::ConsoleMenu::PrintValue(const uint32_t value,
                                  const std::string& desc) {
